...
import cspace.sampling.PathMaker;
...
public class Scene {
  ...
  public void sampleCS() {
    sampledCS = new SampledCSpace(cspace, 
            visuals.subVisuals.getSamplingThreshold(), 
            visuals.subVisuals.getSamplingLength());
  }
  
  public void updatePath() {
    Vec3f s = new Vec3f(
        (float)newPath.waypoints[0].p.x, 
        (float)newPath.waypoints[0].p.y, 
        (float)newPath.waypoints[0].theta);
    Vec3f e = new Vec3f(
        (float)newPath.waypoints[1].p.x, 
        (float)newPath.waypoints[1].p.y, 
